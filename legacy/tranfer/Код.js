...
function openBOMForm() {
  const html = HtmlService.createHtmlOutputFromFile('BOMForm')
    .setWidth(400)
    .setHeight(400);
...
function showForm() {
  const html =
    HtmlService.createHtmlOutputFromFile('Form').setTitle('Додати елемент');
  SpreadsheetApp.getUi().showSidebar(html);
}
...
function addBOMSmart(parentCode, specString, qty) {
  ...
  for (let i = 1; i < bomData.length; i++) {
    if (bomData[i][0] == parent.id && bomData[i][2] == child.id) {
